...
router.get("/dashboard", async(req, res) => {
    ...
    Property.find({UserEmail: userEmail},function(err, properties){
        if(err){
            console.log(err);
        }else{
...
router.get("/rentalCalculator/:id", async(req, res) => {
    ...
    Property.findById({_id: id},function(err, property){
        if(err){
            console.log(err);
        }else{
...
router.post("/rentalCalculator", async(req, res) => {
    ...
    request(options1, function(error, response, body) {
       ...
       // console.log(listings.length);
        ...
        var property_address = property[0].address;
